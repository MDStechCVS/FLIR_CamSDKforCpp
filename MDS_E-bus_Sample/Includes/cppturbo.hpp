...
namespace turbo
{
    ...
    inline constexpr Color<float> GetColor(std::size_t index)
    {
        ...
        return Color<float>(turbo[index][0], turbo[index][1], turbo[index][2]);
    }

    inline constexpr void GetColor(int value, Color<float>& color)
    {
        const std::size_t index = value > 0 ? (value > 255 ? 255 : value) : 0;
        color = GetColor(index);
    }

    inline constexpr void GetColor(double value, Color<float>& color)
    {
        const std::size_t index = value > 0 ? (value > 1 ? 255 : static_cast<int>(value * 255 + 0.5)) : 0;
        color = GetColor(index);
    }

    inline constexpr void GetColor(int value, Color<double>& color)
    {
        const std::size_t index = value > 0 ? (value > 255 ? 255 : value) : 0;
        const Color<float> floats = GetColor(index);
        color = Color<double>(static_cast<double>(floats.r), static_cast<double>(floats.g), static_cast<double>(floats.b));
    }

    inline constexpr void GetColor(double value, Color<double>& color)
    {
        const std::size_t index = value > 0 ? (value > 1 ? 255 : static_cast<int>(value * 255 + 0.5)) : 0;
        const Color<float> floats = GetColor(index);
        color = Color<double>(static_cast<double>(floats.r), static_cast<double>(floats.g), static_cast<double>(floats.b));
    ...
    inline constexpr void GetColor(int value, Color<unsigned char>& color)
    {
        const std::size_t index = value > 0 ? (value > 255 ? 255 : value) : 0;
        const Color<float> floats = GetColor(index);
        color = Color<unsigned char>(static_cast<unsigned char>(floats.r * 255 + 0.5), static_cast<unsigned char>(floats.g * 255 + 0.5), static_cast<unsigned char>(floats.b * 255 + 0.5));
    }

    inline constexpr void GetColor(double value, Color<unsigned char>& color)
    {
        const std::size_t index = value > 0 ? (value > 1 ? 255 : static_cast<int>(value * 255 + 0.5)) : 0;
        const Color<float> floats = GetColor(index);
        color = Color<unsigned char>(static_cast<unsigned char>(floats.r * 255 + 0.5), static_cast<unsigned char>(floats.g * 255 + 0.5), static_cast<unsigned char>(floats.b * 255 + 0.5));
    ...
    inline constexpr void GetColor(int value, Color<int>& color)
    {
        const std::size_t index = value > 0 ? (value > 255 ? 255 : value) : 0;
        const Color<float> floats = GetColor(index);
    ...
    inline constexpr void GetColor(double value, Color<int>& color)
    {
        const std::size_t index = value > 0 ? (value > 1 ? 255 : static_cast<int>(value * 255 + 0.5)) : 0;
        const Color<float> floats = GetColor(index);
        color = Color<int>(static_cast<int>(floats.r * 255 + 0.5), static_cast<int>(floats.g * 255 + 0.5), static_cast<int>(floats.b * 255 + 0.5));
